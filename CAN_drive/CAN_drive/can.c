...
void can_config(can_dtm_canx_enum dtm_canx, uint32_t baudrate_khz)
{
    ...
    // 根据目标波特率计算 prescaler、TSEG1、TSEG2
    // 假设 CAN 时钟 = 80 MHz，采样点 70%，SJW=2
    uint32_t can_clock_khz = 80000;   // 80 MHz
    uint32_t tseg1 = 13;              // 固定 TSEG1=13, TSEG2=6 时位时间=20
    uint32_t tseg2 = 6;
    uint32_t total_tq = tseg1 + tseg2 + 1; // 20
   ...
   */
    ...
    /* setup a filter for message ID 如果要过滤指定 ID，可配置 filter_element*/
 ...
 /* nvic_irq_enable(DTM_CAN0_INT0_IRQn, 0, 0);
    nvic_irq_enable(DTM_CAN2_INT0_IRQn, 0, 0);
    nvic_irq_enable(DTM_CAN4_INT0_IRQn, 0, 0);
...
void canfd_config(void)
{
    ...
    /* CAN baudrate = 500 Kbps, 70%, based on CAN clock = 80MHz  80/（8*（1+13+6））*/
            //can_param.prescaler = 8;
            //can_param.resync_jump_width = 2;
            //can_param.time_segment_1 = 13;
           // can_param.time_segment_2 = 6;

    /* CAN baudrate = 1M Kbps, 70%, based on CAN clock = 80MHz */
    can_param.prescaler = 4;
    can_param.resync_jump_width = 2;
    can_param.time_segment_1 = 13;
    can_param.time_segment_2 = 6;
...
ErrStatus can_send_std_frame(can_dtm_canx_enum dtm_canx, uint32_t std_id, uint8_t *data, uint8_t len)
{
    ...
    uint8_t mailbox = can_message_transmit_prepare(dtm_canx, &tx_msg);
    if(mailbox == 0xFF) {
        return ERROR;
    }
...
void can_enable_rx_interrupt(can_dtm_canx_enum dtm_canx, uint8_t irqn_priority)
{
    ...
    // 使能 NVIC
  //  nvic_irq_enable(DTM_CAN5_INT0_IRQn, irqn_priority, 0);   // 根据 CAN 实例选择 IRQn
    if(dtm_canx==DTM_CAN0)  nvic_irq_enable(DTM_CAN0_INT0_IRQn, irqn_priority, 0); 
    if(dtm_canx==DTM_CAN2)  nvic_irq_enable(DTM_CAN2_INT0_IRQn, irqn_priority, 0); 
    if(dtm_canx==DTM_CAN4)  nvic_irq_enable(DTM_CAN4_INT0_IRQn, irqn_priority, 0);  
    if(dtm_canx==DTM_CAN5)  nvic_irq_enable(DTM_CAN5_INT0_IRQn, irqn_priority, 0);  
}

// 以 DTM_CAN5 为例
void DTM_CAN5_INT0_IRQHandler(void)
{
    // 检查是否为 FIFO0 新消息中断
    if(can_interrupt_flag_get(DTM_CAN5, CAN_INT_FLAG_RFIFO0_NEW)) {
        // 清除中断标志
        can_interrupt_flag_clear(DTM_CAN5, CAN_INT_FLAG_RFIFO0_NEW);
        
        can_receive_message_struct rx_msg;
        can_struct_para_init(CAN_RX_MESSAGE_STRUCT, &rx_msg);
        
        // 从 FIFO0 接收消息
        can_message_receive(DTM_CAN5, CAN_RXFIFO0, &rx_msg);
        
        // 处理接收到的数据
        if(rx_msg.xtd == CAN_FF_STANDARD) {   // 标准帧
            // 用户处理：例如保存到队列、设置标志等
            // rx_msg.id, rx_msg.data, rx_msg.data_bytes
        }
    }
}
// 以 DTM_CAN4 为例
...
// 以 DTM_CAN2 为例
void DTM_CAN2_INT0_IRQHandler(void)
{
    // 检查是否为 FIFO0 新消息中断
    if(can_interrupt_flag_get(DTM_CAN2, CAN_INT_FLAG_RFIFO0_NEW)) {
        // 清除中断标志
        can_interrupt_flag_clear(DTM_CAN2, CAN_INT_FLAG_RFIFO0_NEW);
        
        can_receive_message_struct rx_msg;
        can_struct_para_init(CAN_RX_MESSAGE_STRUCT, &rx_msg);
        
        // 从 FIFO0 接收消息
        can_message_receive(DTM_CAN2, CAN_RXFIFO0, &rx_msg);
...
{
    // 检查是否为 FIFO0 新消息中断
    if(can_interrupt_flag_get(DTM_CAN0, CAN_INT_FLAG_RFIFO0_NEW)) {
        // 清除中断标志
        can_interrupt_flag_clear(DTM_CAN0, CAN_INT_FLAG_RFIFO0_NEW);
        
        can_receive_message_struct rx_msg;
        can_struct_para_init(CAN_RX_MESSAGE_STRUCT, &rx_msg);
        
        // 从 FIFO0 接收消息
        can_message_receive(DTM_CAN0, CAN_RXFIFO0, &rx_msg);
