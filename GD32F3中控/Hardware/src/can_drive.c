...
void CAN_Init_Config(void)
{
    ...
    /* 5. 动态计算 prescaler 匹配 500Kbps */
    {
        uint32_t apb1_clk = rcu_clock_freq_get(CK_APB1);
        const uint32_t total_tq = 1U + 14U + 5U;  /* SJW=1 + BS1=14 + BS2=5 = 20 */
        uint32_t prescaler = apb1_clk / (500000U * total_tq);
        if (prescaler == 0U) prescaler = 1U;
        uint32_t actual_baud = apb1_clk / (prescaler * total_tq);
        
        printf("[CAN] APB1=%luHz prescaler=%lu actual=%lu bps\r\n",
               (unsigned long)apb1_clk, (unsigned long)prescaler, (unsigned long)actual_baud);
